...
\begin{frame}
  ...
  \begin{itemize}
  \item Deals with \emph{backing stores}
    \begin{itemize}
    \item low-level blocks of memory that underlie memory areas
    \end{itemize}
  % \item The \emph{root
  %     backing store} covers the whole of backing store memory.
  \item Backing stores may be nested.
  \item Operations on backing stores:\newline
    clearing, resizing, allocating, locating an object's backing store
    % \begin{itemize}
    ...
    % \end{itemize}
  \item Also manages stacks, separately from backing stores
  \end{itemize}
  \vfill
  \begin{tikzpicture}
    \scriptsize
    \coordinate (width)  at (9,0);
    \coordinate (height) at (0,2);

    \path (0,0) --
    coordinate[pos=0.00] (stack bot)
    coordinate[pos=0.01] (stack1 bot)
    coordinate[pos=0.05] (MM bot)
    coordinate[pos=0.10] (PRM bot)
    coordinate[pos=0.15] (NPM bot)
    coordinate[pos=0.20] (stack1 top)
    coordinate[pos=0.21] (stack2 bot)
    coordinate[pos=0.40] (stack2 top)
    coordinate[pos=0.41] (stack3 bot)
    coordinate[pos=0.60] (stack3 top)
    coordinate[pos=0.85] (NPM top)
    coordinate[pos=0.90] (PRM top)
    coordinate[pos=0.95] (MM top)
    coordinate[pos=1.00] (stack top)
    (height);

    \path (0,0) --
    coordinate[pos=0.34] (RBS mid)
    coordinate[pos=0.35] (MM left)
    coordinate[pos=0.50] (MM mid)
    coordinate[pos=0.51] (PRM1 left)
    coordinate[pos=0.675] (PRM1 mid)
    coordinate[pos=0.68] (NPM left)
    coordinate[pos=0.80] (NPM right)
    coordinate[pos=0.81] (PRM1 right)
    coordinate[pos=0.82] (PRM2 left)
    coordinate[pos=0.98] (PRM2 right)
    coordinate[pos=0.99] (MM right)
    coordinate[pos=1.01] (stack left)
    coordinate[pos=1.015] (stacks left)
    coordinate[pos=1.195] (stacks right)
    coordinate[pos=1.20] (stack right)
    (width);
    
    \draw (0,0) rectangle (width |- height);
    \draw[dotted] (RBS mid |- 0,0) rectangle (RBS mid |- height);
    \path (0,0) rectangle (MM left |- height) 
    node[pos=0.5,align=center] {Root Backing\\Store\\(Immortal Memory)};
    
    \draw (MM left |- MM bot) rectangle (MM right |- MM top);
    \draw[dotted] (MM mid |- MM bot) -- (MM mid |- MM top);
    \path (MM left |- MM bot) rectangle (PRM1 left |- MM top)
    node[pos=0.5,align=center] {Mission\\Memory};
    
    \draw (PRM1 left |- PRM bot) rectangle (PRM1 right |- PRM top);
    \draw[dotted] (PRM1 mid |- PRM bot) -- (PRM1 mid |- PRM top);
    \path (PRM1 left |- PRM bot) rectangle (NPM left |- PRM top)
    node[pos=0.5,align=center] {Per-release\\Memory};

    \draw (PRM2 left |- PRM bot) rectangle (PRM2 right |- PRM top);
    \path (PRM2 left |- PRM bot) rectangle (PRM2 right |- PRM top)
    node[pos=0.5,align=center] {Per-release\\Memory};
    
    % \draw (6.5,0.3) rectangle (11.5,1.4);
    % \node[align=center] at (9,1.2) {Per-release Memory};
      
    \draw (NPM left |- NPM bot) rectangle (NPM right |- NPM top)
    node[pos=0.5,align=center]{Private\\Memory};

    \draw (stack left |- stack bot) rectangle (stack right |- stack top);
    \path (stack left |- stack3 top) rectangle (stack right |- stack top)
    node[pos=0.5,align=center]{Stack\\Space};

    \draw (stacks left |- stack1 bot) rectangle (stacks right |- stack1 top);
    \draw (stacks left |- stack2 bot) rectangle (stacks right |- stack2 top);
    \draw (stacks left |- stack3 bot) rectangle (stacks right |- stack3 top);
...
\begin{frame}
  ...
  \begin{itemize}
  \item The SCJVM schedules abstract \emph{threads}.
  % \item Threads transition between different states.
  \item Scheduled using priority queues
    \begin{itemize}
    \item $current$ is front of the highest-priority queue.
    \end{itemize}
  \item Implements priority-ceiling emulation
  \item Interrupts scheduled as threads
  \end{itemize}
  
  \begin{tikzpicture}
    \usetikzlibrary{positioning}
    \node (free)    at (0,0)        {\small $free$};
    \node (created) [right=of free] {\small $created$};
...
\begin{frame}
  ...
  \begin{itemize}
  \item Supplies the current time
  \item Receives clock tick interrupts from hardware
  \item Allows an alarm to be set for a specific time
    \begin{itemize}
    \item sends an interrupt to the Scheduler
    \item drives event handler execution
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item Process' state contains its program counter and frame stack.
  \item {The Interpreter is a parallel composition of these threads.
      ...
      \end{circus}}
  \item The thread processes proceed through several different states
    \begin{itemize}
      \item roughly corresponding to the Scheduler thread states
    \end{itemize}
    % different
    % actions.
    %as the scheduler starts and switches between them.
  \end{itemize}
  \centering
  \begin{tikzpicture}
    \node at (9cm,0) (width) {};
