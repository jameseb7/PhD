...
They include such areas as low level memory management and
scheduling.

A model of the core execution environment will be presented in 
Chapter~\ref{cee-chapter}.
In this chapter, we will instead focus on the virtual machine
services.

As indicated in Figure~\ref{scjvm-services-fig}, we divide the SCJVM
services into three areas:~memory management, scheduling, and the
real-time clock.
We will present a \Circus{} model of these areas in the separate
sections of this chapter, with a \Circus{} process for each of the three
areas.
A model of the memory manager will be presented in
Section~\ref{memory-manager-section}, followed by a model of the
scheduler in Section~\ref{scheduler-section}, then the real-time
clock in Section~\ref{realtime-clock-section}.
Finally, the \Circus{} processes are combined into a single process in
Section~\ref{scjvm-services-section}.

An earlier version of this model was presented at the %TODO: reference conference
