...
to facilitate the development of such a machine.  We could attempt to verify an
existing SCJ virtual machine but they tend to be too complex to verify easily,
and would require many changes as errors were discovered and fixed.  Instead, it
seems better to construct a specification of the requirements for an SCJ virtual
machine, from which an implementation of an SCJ virtual machine could be
constructed, and show that to be correct.  The specification must cover the
whole of the virtual machine, including both the execution of Java bytecode by
compiling it to native code and the supporting services that the virtual machine
must offer.

In order to construct the specification, we must identify the requirements of an
SCJ virtual machine, including the semantics of Java bytecode, and show a
correct compilation to native code.  The specification must be put in terms of
some formal specification language with well-defined laws to facilitate
reasoning about it.  The proof of the specification's correctness should be
machine-checked to provide certainty that there have been no errors in the
writing of the proof.

The requirements of an SCJ virtual machine are obtained from the requirements
imposed, both explicitly and implicitly, on virtual machines by the SCJ
specification.  It is also helpful to consider the approach taken by some
existing SCJ virtual machines on points where the SCJ specification is unclear.
There has already been work done on the formal semantics of Java
bytecode~\cite{bertelsen2000, jones1998, stark2001}, which can be used to
construct a formal model for the compilation to native code.
...
There have been various formal languages that have been developed for specifying
systems.  We will use the \Circus{} specification language~\cite{oliveira2009},
as it has been used in some of the previous work on formalising
SCJ~\cite{cavalcanti2011, cavalcanti2013}. \Circus{} is based on
CSP~\cite{roscoe2011} and the Z notation~\cite{woodcock1996}, using CSP to
specify processes, and using Z to specify state and data operations.  The
semantics of \Circus are well-defined and have been formalised using the
Unifying Theories of Programming (UTP) model~\cite{hoare1998}.

There are various proof assistants that can be used for mechanised proof of
\Circus{} specifications but we will focus on using the Isabelle proof
assistant~\cite{nipkow2014} as it has existing formalisations of
\Circus{}~\cite{joshi2012} and the UTP~\cite{foster2015}, and it has been used
to verify compilers for Java-like languages in the past~\cite{klein2006,
  nipkow2000, lochbihler2010}.

In pursuit of these objectives, we have already produced a specification of the
requirements for the services of an SCJ virtual machine, with a formal model in
\Circus{}.
