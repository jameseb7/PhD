...
numkber of Ada programmers is not very large~\cite{bissyande2013}.  While Java has not
...
compiler correctness techniques must be considered in verfication of such a
...
targetseveral intermediate machines in the proof of correctness. Viewing the
...
the proof was feasable and could be completed relatively easily. A point to note
...
from the machine-checked proofs.  Code generation facitilies are available with
...
% * the overall aim is to specify a framework for verification of SCJ virtual
% machines 

% * we have identified an SCJVM as requiring the component shown in the
% diagram
