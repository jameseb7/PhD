...
\begin{slide}{Early Work}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \node[align=center] (T1) at (4cm,2.5cm) {stack machine\\code};
      \node[align=center] (U1) at (4cm,0cm)   {stack machine\\states};
      \node[align=center] (T2) at (8cm,2.5cm) {stack machine\\code};
      \node[align=center] (U2) at (8cm,0cm)   {stack machine\\states};
...
\begin{slide}{Algebraic Approach}
  ...
  \end{itemize}
  % \begin{tikzpicture}
  %   \draw (0cm,0cm) |- (10cm,4cm) |- (0cm,0cm);
  %   \draw (5cm,3.2cm) node[align=center] {Handel-C + Specification Constructs};
    
  %   \draw (2cm,2cm)   circle[radius=1.5cm]               node[align=center] {Handel-C\\Program};
  %   \draw (8cm,2cm)   circle[radius=1.5cm] ++(0cm,1cm) node[align=center] {Hardware};
  %   \draw (8cm,1.8cm) circle[radius=1cm]                 node[align=center] {Hardware\\components};
    
  %   \path (3.5cm,2cm) edge[->] node[align=center, above] {\LARGE $\sqsubseteq$} node[align=center, below] {Refinement\\Strategy\\(Compilation)} (6.5cm,2cm);
  % \end{tikzpicture}      
\end{slide}
