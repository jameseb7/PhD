...
Rule~[\nameref{introduce-class-information-rule}], shown below.
...
simply replacing the call to the $Invoke$ action with actions that
specify the behaviour for the special method, and introducing a $pc$
...
method call with the action defining the method's behaviour.
...
the choice in the $Invoke$ action, replacing it with a call to
$InvokeOther$, sequenced with the action $Poll$ and the method action,
$M$, defining the method's behaviour.
