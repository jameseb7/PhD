...
Rule~[\nameref{refine-invokespecial-rule}] and
Rule~[\nameref{refine-invokevirtual-single-rule}] are similar, and can
be found in Appendix~\ref{compilation-rules-appendix}.
...
the current class.
The $c_0$ information is obtained from the $pc$ value, $j$, when the
...
\begin{restatable}[refine-invokestatic]{crule}{RefineInvokestaticRule}
  ...
  \begin{displaymath}
    ...
    \t1 (\exists m_0 : MethodID | m_0 \in \dom c_0.methodEntry @ j \in c_0.methodEntry~m_0 \upto c_0.methodEnd~m_0) \\
...
method call.
There are two rules for this in the case of method calls with a single
target, applied in another try block on
...
over those targets using Rule~\ref{refine-invokevirtual-multi-rule},
which is applied on line~\ref{algorithm-refine-invokevirtual-multi}.
...
In our example, the $pc = 14$ branch, shown earlier, has
$HandleInvokeVirtualEPC(36)$ at a point when the value of $pc$ is
$16$.
This falls within the $methodEntry$ and $methodEnd$ values for
$handleAsyncEvent$ in the $TPK$ class information shown in
Figure~\ref{example-model-figure}.
The constant pool index $36$ is thus looked up in its $constantPool$,
to yield a $MethodRef$ whose class identifier is that of the
\texttt{ConsoleConnection} class and whose method identifier is that
of \texttt{openOutputStream()}.
The \texttt{ConsoleConnection} class represents a connection to an
input/output console, and there is only one class to represent that
type of connection.
This means it has no subclasses, and so the method call has a single
target.
Since the instruction being handled is \texttt{invokevirtual}, we
apply Rule~[\nameref{refine-invokevirtual-single-rule}] to refine the
branch to the form shown below.
\begin{circusaction}
  \t2 {} \cdots {} \\
  \t2 {} \circelse pc = 14 \circthen \cdots \circseq pc := 16 \circseq Poll \circseq \\
  \t3 \circvar poppedArgs : \seq Word \circspot \\
  \t3 \lschexpract \exists argsToPop == methodArguments~m @ InterpreterStackFrameInvoke \rschexpract \circseq \\
  \t3 Invoke(ConsoleConnectionClassID, openOutputStream, poppedArgs, \false) \\
  \t2 {} \cdots {}
\end{circusaction}
This method is not a special method, so it is resolved using
Rule~[\nameref{resolve-normal-method-rule}].
The method \texttt{openOutputStream()} is defined in the
\texttt{ConsoleConnection} class itself as it returns an
\texttt{OutputStream} specific to that class.
Thus the calculation of the method's entry point using $ResolveMethod$
is fairly straightforward.
It results in a $pc$ value at which a method action
$ConsoleConnection\_openOutputStream$ occurs.
That is therefore sequentially composed with the $Invoke$ action,
which is specialised to $InvokeOther$.
That results in the sequence of actions shown below.
\begin{circusaction}
  \t2 {} \cdots {} \\
  \t2 {} \circelse pc = 14 \circthen \cdots \circseq pc := 16 \circseq Poll \circseq \\
  \t3 \circvar poppedArgs : \seq Word \circspot \\
  \t3 \lschexpract \exists argsToPop == methodArguments~m @ InterpreterStackFrameInvoke \rschexpract \circseq \\
  \t3 InvokeOther(ConsoleConnectionClassID, openOutputStream, poppedArgs, \false) \circseq \\
  \t3 Poll \circseq ConsoleConnection\_openOutputStream \circseq pc := 17 \\
  \t2 {} \cdots {}
\end{circusaction}
This can then be sequenced with the instructions at $pc = 17$.
