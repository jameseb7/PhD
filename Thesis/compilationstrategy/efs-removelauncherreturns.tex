...
\begin{algorithm}[ht]
  \begin{algorithmic}[1]
    ...
    \State \Match{$methodBody$}{($A \circseq returnAction$)}
    ...
    \State \MatchIn{$\begin{array}{l}(\circvar retVal : Word \circspot (A)(c, m, a, retVal) \circseq \\\t1 executeMethodRet!thread!retVal \then \Skip)\end{array}$}{\Call{ActionBody}{Started}}
...
\begin{algorithm}[ht]
  \begin{algorithmic}[1]
    \For{$branch \gets$ \Call{MethodBranches}{$M$}}
    \If{\Matches{$branch$}{$B \circseq HandleReturnEPC$}}
    \State \Return{$HandleReturnEPC$}
    \ElsIf{\Matches{$branch$}{$B \circseq HandleAreturnEPC$}}
    \State \Return{$HandleAreturnEPC$}
    \EndIf
    \EndFor
    \State \Return{$HandleReturnEPC$}
  \end{algorithmic}
  \caption{MethodReturnAction($M$)}
\end{algorithm}
\begin{algorithm}[ht]
  \begin{algorithmic}[1]
    \State $branches \gets \langle\rangle$
    \If{\Matches{$M$}{$(\circif {} \circelse i \in I \circspot g_i \circthen A_i \circfi)$}}
    \For{$i \gets I$}
    \State $branches \gets branches \cat$\Call{MethodBranches}{$A_i$}
    \EndFor
    \Else
    \State $branches \gets \langle M \rangle$
    \EndIf
    \State \Return $branches$
  \end{algorithmic}
  \caption{MethodBranches($M$)}
\end{algorithm}
