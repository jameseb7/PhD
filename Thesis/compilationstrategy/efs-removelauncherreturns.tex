...
\begin{algorithm}[ht]
  \begin{algorithmic}[1]
    \For{$m \gets methods$}
    \State \ApplyToFor{Law~[\nameref{rec-action-intro-law}]}{$m$, wherever an infinite loop is not followed by a return}{a return action that matches those in other branches of $m$}
    \label{algorithm-introduce-infinite-loop-returns}
    \State \ApplyTo{Law~[\nameref{alt-seq-dist-law}]}{$m$, until a single return action is at the end of $m$}
    \label{algorithm-distribute-returns}
    \State \ApplyFor{Law~[\nameref{copy-rule-law}]}{all occurrences of $m$}
    \label{algorithm-eliminate-returns-copy-method-in}
    \State \ApplyReverseFor{Law~[\nameref{action-intro-law}]}{$m$}
    \State \ApplyFor{Law~[\nameref{action-intro-law}]}{$m$, with return actions excluded}
    \State \ApplyReverseFor{Law~[\nameref{copy-rule-law}]}{all occurrences of $m$}
