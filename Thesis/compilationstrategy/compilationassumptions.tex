...
\added{
Additionally, we note a few points where, while the correct operation
of the compilation strategy is not affected, the semantics of the
generated code differs from the usual semantics since we follow the
approach of icecap.
It is important that we follow the approach of a practical tool on
this, to ensure that our strategy can be implemented.
In particular, we follow icecap's approach in compiling addition in
Java bytecode to addition in C, making the assumption that the
addition does not overflow.
This means that Java code input to the strategy must avoid integer
overflow in order to be MISRA-C compilant, since signed integer
overflow is undefined behaviour in C, and behaviours that are
undefined in C must not be relied on in MISRA-C.
We believe the structure of the compilation strategy and the
restrictions described in this section are sufficient to ensure that
the resultant code is consistent with MISRA-C.
}
...
\moved{
We \added{also} do not handle synchronisation of static methods, since
