...
\begin{figure}[t]
  ...
  \begin{circus}
    ...
    \t3 \circvar value1, value2 : Word \circspot InterpreterPop2EPC \circseq pc := \IF value1 \leq value2 \THEN 32 \ELSE 28 \\
...
\begin{figure}[t!]
  ...
  \begin{circusaction}
    ...
    \t1 \circblockbegin
    executeMethod? t \prefixcolon (t = thread)?c?m?a \then ExecuteMethod(c,m,a) \circseq X \\
    {} \extchoice {} \\
  ...
  \begin{circusaction}
    Started \circdef \\
    \t1 \circblockbegin
    executeMethod? t \prefixcolon (t = thread)?c?m?a \then ExecuteMethod(c,m,a) \circseq \\
    \t1 \circblockbegin
