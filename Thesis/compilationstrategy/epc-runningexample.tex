...
Figure~\ref{example-model-figure}, along with\added{ the} $TPK$ class
information.
...
classes (since the superclasses of \texttt{ManagedEventHandler} do not
contribute any fields), whose $Class$ data structures are presented in
Figure~\ref{example-superclasses-model-figure}.\addchange{Clarified
  elision in
  Figure\protect~\protect\ref{example-superclasses-model-figure}}\added{
  We omit the information not involved in determining the fields of
  those classes.}
...
\begin{figure}[t!]
  ...
  \begin{circus}
    ...
    \t3 \lschexpract InterpreterPopEPC[value1!/value!] \rschexpract \circseq pc := \IF value1 \leq value2 \THEN 21 \ELSE 42 \\
    \t2 {} \cdots {} \\
    % \t2 {} \circelse pc = 40 \circthen HandleIconstEPC(10) \circseq pc := 41 \\
...
On line~\ref{algorithm-introduce-forward-sequence} of\deleted{ the}
...
\added{In this figure, and the other figures in this chapter, we omit
  the branches of the choice in $Running$ that cannot be reached from
  the start of a method, since the instructions in those branches are
...
we define the \Call{IntroduceSequentialComposition}{} procedure.
...
\begin{figure}[tp!]
  ...
  \begin{circus}
    ...
    \t3 Poll \circseq (\circvar poppedArgs : \seq Word \circspot \lschexpract \exists argsToPop?  == 6 @ InterpreterStackFrameInvokeEPC \rschexpract \circseq \\
...
loop\deleted{ on}\added{ in} lines~\ref{algorithm-method-loop}\added{
...
of\deleted{ this algorithm}\added{ Algorithm~\ref{epc-algorithm}}.
...
\begin{figure}[tp!]
  ...
  \begin{circus}
    ...
    \t3 \lschexpract InterpreterPopEPC[value1!/value!] \rschexpract \circseq pc := \IF value1 \leq value2 \THEN 32 \ELSE 28 \\
    % \t2 {} \cdots {} \\
...
algorithm, and\added{ its result} can be seen in
...
\addchange{Changed explanation of
  Figure\protect~\protect\ref{final-method-separation-example-figure}
  to reflect changes}%
\added{%
...
Figure~\ref{loop-and-conditional-introduction-example-figure}.
The $pc$ values for the other branches are now redundant, since their
instructions have been folded into the method actions.
}
\deleted{
...
\begin{figure}[tp!]
  ...
  \addchange{Inserted full definition of $TPK\protect\_handleAsyncEvent$ in
...
The next step is then to eliminate the\deleted{se} redundant paths and
...
\deleted{This can be seen in
}Figure~\ref{refine-main-actions-example-figure}\deleted{, which}
