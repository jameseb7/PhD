...
conditional (Figure~\ref{if-else-graph}), although it is constructed
from such a graph.
...
The sequence is constructed such that, where a node occurs in the
sequence, the successors of that node (ignoring loops) must occur
earlier in the sequence than the node itself.
