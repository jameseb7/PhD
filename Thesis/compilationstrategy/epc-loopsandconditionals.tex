...
the previous section, but it just constructs the control flow graph
...
In our rules for introducting the structures, we instead leave a $pc$
assignment after the structure that is used to introduce the
sequential composition separately later on (we introduce the
sequential composition on
line~\ref{algorithm-lci-sequence-introduction} of the algorithm).
\begin{center}
  \begin{tikzpicture}
...
sequential composition can be introduced as discussed above.
...
Note that Rule~[\nameref{conditional-introduction-rule}] simply
introduces a conditional with no restrictions on its form.
...
Rule~[\nameref{while-introduction-rule2}], that introduces the loop in
the true branch of the conditional.
...
control flow graph of the structure to either a sequential composition
graph (Figure~\ref{sequence-figure}) or a single node.
Divergent conditionals and infinite loops are the only structures
whose control flow graphs are reduced to a single node by their
introduction rules.
...
is embedded in another graph through end node replacement or internal
node replacement.
