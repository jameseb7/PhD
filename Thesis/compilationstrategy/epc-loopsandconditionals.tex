...
We iterate over the nodes in the methods control flow graph,
...
structures, structures must be introduced for the the sucessors of a
node before they can be introduced for the node itself.
We specify this using the procedure \Call{ReverseNodes}{$cfg$}, which
constructs a sequence indicating the order in which the nodes should
be iterated over.

\Call{ReverseNodes}{$cfg$} begins by constructing a set of nodes that
can occur next in the order.
This is initially set to the end nodes of $cfg$ (ignoring looping
edges for the purposes of determining end nodes).
A node is removed from the set (the choice of node is
nondeterministic, since the order does not matter among the nodes that
can occur next), and added to the end of the sequence.
Each of the predecessors of the chosen node (ignoring looping edges)
are then added to the set, if their successors already occur in the
sequence.
The \Call{ReverseNodes}{$cfg$} procedure then loops, choosing a new
node from the set, until there are no more nodes to consider in the
set.
This loop terminates since looping edges are ignored in the
control-flow graph.

In our example, we may consider the $pc=42$ and $pc=35$ nodes first,
then $pc=28$ and $pc=32$, then $pc=21$, $pc=39$, and finally $pc=7$.
This ensures the sucessors of each node (ignoring loops) are
introduced before the node itself, as can be seen from the graph in
...
without affecting the validity of the strategy.
