...
checks performed on Java class files, the object fields of each class
are defined by a straight hierarchy of superclasses.
We can thus assume the fields of an object's superclass are stored
first in its representation, before the fields specific to its own
type.
This ensures that the definition of the structs is consistent with the
way struct casting works in C.
...
This means that $TPKObj$ values can be easily converted to those
types.
Other aperiodic event handlers may have additional fields to store
data specific to them.
Those fields would be placed at the end of their types, which can be
converted to $AperiodicEventHandlerObj$ by simply discarding the
additional fields.
...
$ManagedEventHandlerObj$ can be mapped onto it by the function
$castManagedEventHandler$, representing a C cast to that type.
Similar functions are defined for the other object types.
...
These types and functions are all used in the struct manager after the
...
The $staticClassFields$ component in $StructManState$ is of the
