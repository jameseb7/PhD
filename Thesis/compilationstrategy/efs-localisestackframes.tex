...
\begin{restatable}[$InterpreterReturn$-args-intro]{crule}{ArgumentsIntroductionRule}
  ...
  $arg{<}n{>}$ are not free in $M$\addchange{Added needed provisio to
  Rule\protect~[\protect\nameref{arguments-introduction-rule}]}\deleted{and are distinct from $c$,
  $m$ and $args$, then}\added{, are distinct from $c$, $m$ and $args$,
  and $\# args = n$}, then
  \begin{circus}
    \begin{array}{l}
