...
\begin{algorithm}
  \begin{algorithmic}[1]
    ...
    \end{array}$ 
    \State \ExhaustivelyApplyFor{Rule~[\nameref{HandleReturnEPC-stackFrame-introduction-rule}]}{$methodBody$}
    ...
    \MatchThen{$(\circvar stackFrame : StackFrameEPC \circspot init \circseq newMethodBody)$}
    ...
    \State \ApplyFor{Law~[\nameref{action-intro-law}]}{$methodName$,
      $\begin{array}[t]{l}
         ...
         \t4 init \circseq newMethodBody
       \end{array}$}
     \label{algorithm-localise-stack-frames-method-reintroduction}
     \ExhaustivelyApplyReverseFor{Law~[\nameref{copy-rule-law}]}{$methodName$}
