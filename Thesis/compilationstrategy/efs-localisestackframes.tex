...
\begin{algorithm}
  \begin{algorithmic}[1]
    ...
    \State $newMethodBody \gets
    methodBody\begin{array}[t]{l}[stackFrame/last~frameStack, \\
                stackFrame'/last~frameStack']
              \end{array}$
    \State $numArgs \gets$ \Call{MethodArguments}{$methodName$}
    \State \ExhaustivelyApplyFor{Rule~[\nameref{HandleReturnEPC-stackFrame-introduction-rule}]}{$methodBody$, $numArgs$}
    \label{algorithm-HandleReturnEPC-stackFrame-introduction-rule}
    % TODO: add areturn handling rules
    \State \ApplyReverseFor{Law~[\nameref{action-intro-law}]}{methodName, \Call{ActionBody}{methodName}}
    \label{algorithm-localise-stack-frames-method-elimination}
    \MatchThen{$\begin{array}[t]{l}
                  (\circval arg1, \ldots, arg{<}numArgs{>} : Word \circspot \\
                  \t1 \circvar stackFrame : StackFrameEPC \circspot init \circseq newMethodBody)
                \end{array}$}
    \label{algorithm-localise-stack-frames-method-reintroduction-match}
    \State \ApplyFor{Law~[\nameref{action-intro-law}]}{$methodName$,
      $\begin{array}[t]{l}
         \circval arg1, \ldots, arg{<}numArgs{>} : Word \circspot \\
         \t1 \circvar stackFrame : StackFrameEPC \circspot \\
         \t1 init \circseq newMethodBody
       \end{array}$}
     \label{algorithm-localise-stack-frames-method-reintroduction}
...
Rule~[\nameref{HandleReturnEPC-stackFrame-introduction-rule}], shown
in Figure~\ref{HandleReturnEPC-stackFrame-introduction-rule-figure}.
This rule is applied to two parameters:~$methodBody$, the body of
$methodName$, and $numArgs$, the number of arguments to the method
(which is encoded as part of the method identifier, and so can be
determined during the strategy).
We apply this rule wherever possible on
line~\ref{algorithm-HandleReturnEPC-stackFrame-introduction-rule}.

\begin{figure}[thp]
\begin{restatable}[$Return$-$stackFrame$-introduction]{crule}{HandleReturnEPCStackFrameIntroductionRule}
  \label{HandleReturnEPC-stackFrame-introduction-rule}
  %\setlength{\zedtab}{0.4cm}
  %\setlength{\zedindent}{0pt}
  %\setlength{\zedleftsep}{0pt}
  Given a sequence of actions $A$ and $n : \nat$, if $A$ operates solely on
...
Rule~[\nameref{HandleReturnEPC-stackFrame-introduction-rule}] also
introduces the arguments to the method as value parameters, with their
values passed into the action initialising the $stackFrame$ variable.
...
method body (including the initialisation of $stackFrame$) into its
own action.
We thus redefine the method action $methodName$ to correspond to the
parametrised block introduced by
Rule~[\nameref{HandleReturnEPC-stackFrame-introduction-rule}].
This is done using Law~[\nameref{action-intro-law}] to eliminate the
...
Rule~[\nameref{HandleReturnEPC-stackFrame-introduction-rule}] for
method bodies followed by $InterpreterAreturn1$ and
$InterpreterAreturn2$, which would introduce a result parameter for
the method in addition to the value parameters representing the
method's arguments.
