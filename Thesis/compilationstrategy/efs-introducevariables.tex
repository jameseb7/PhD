...
\begin{algorithm}
  \begin{algorithmic}[1]
    ...
    \State \ApplyTo{Rule~[\nameref{refine-PutfieldSF-rule}]}{all occurrences of $PutfieldSF$}
    \label{algorithm-apply-refine-PutfieldSF}
    \State \ApplyTo{Rule~[\nameref{refine-GetfieldSF-rule}]}{all occurrences of $GetfieldSF$}
    \State \ApplyTo{Rule~[\nameref{refine-PutstaticSF-rule}]}{all occurrences of $PutstaticSF$}
    \State \ApplyTo{Rule~[\nameref{refine-GetstaticSF-rule}]}{all occurrences of $GetstaticSF$}
    \State \ApplyTo{Rule~[\nameref{refine-NewSF-rule}]}{all occurrences of $NewSF$}
    ...
    \EndFor
    \State \Apply{Rule~[\nameref{eliminate-value1-value2-conditional-rule}]}{wherever applicable}
    \label{algorithm-apply-eliminate-value1-value2-conditional-rule}
    \State \Apply{Rule~[\nameref{eliminate-oid-getField-rule}]}{wherever applicable}
    \State \Apply{Rule~[\nameref{eliminate-oid-value-putField-rule}]}{wherever applicable}
    \State \Apply{Rule~[\nameref{eliminate-value-putStatic-rule}]}{wherever applicable}
    \label{algorithm-apply-eliminate-value-putStatic-rule}
    \State \ApplyTo{Rule~[\nameref{method-parameter-introduction-rule}]}{each applicable method call}
    \label{algorithm-method-parameter-introduction}
    \State \ApplyTo{Rule~[\nameref{getClassIDOf-method-parameter-introduction-rule}]}{each applicable method call}
    \State \ApplyTo{Rule~[\nameref{getClassIDOf-multi-method-parameter-introduction-rule}]}{each applicable method call}
    \State \ApplyTo{Rule~[\nameref{toplevel-method-parameter-introduction-rule}]}{each applicable method call}
    ...
    \State \ApplyFor{Law~[\nameref{action-intro-law}]}{$m$, with the actions in the corresponding parametrised block}
