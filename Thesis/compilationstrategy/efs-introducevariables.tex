...
every method call in $methodBody$.
...
\begin{figure}[thp]
  \begin{restatable}[$poppedArgs$-elimination]{crule}{MethodParameterIntroductionRule}
...
argument parameters.
This is performed by first replacing every call to $methodName$ with
its definition using Law~[\nameref{copy-rule-law}], on
line~\ref{algorithm-introduce-variables-copy-method-in}.
Then, we apply Rule~[\nameref{argument-variable-elimination-rule}] on
line~\ref{algorithm-argument-variable-elimination}.
This rule eliminates the parameters $arg1, \ldots, arg{<}n{>}$,
introduced in Section~\ref{localise-stack-frames-subsection},
replacing them with the first $n$ local variables,
$var1, \ldots, var{<}n{>}$.
  ...
  \begin{restatable}[argument-variable-elimination]{crule}{ArgumentVariableEliminationRule}
    ...
    \begin{circus}
      \begin{array}{l}
        ...
        \t1 \circvar var1, \ldots, var{<}\ell{>} : Word \circspot \\
        \t1 \circvar stack1, \ldots, stack{<}s{>} : Word \circspot \\
        \t2 
...
a value.
Handling methods that do return a value would require another
compilation rule, similar to
Rule~[\nameref{argument-variable-elimination-rule}], which would
...
After the method's parameters have been transformed, we replace the
definition of the $methodName$ action with the new method body, which
is the parametrised block created by
Rule~[\nameref{argument-variable-elimination-rule}].
The old definition of $methodName$ is eliminated on
line~\ref{algorithm-efs-eliminate-old-method-action} and the new
action is obtained by matching the form of the parametrised block on
line~\ref{algorithm-efs-match-new-method-action} and introducing it on
line~\ref{algorithm-efs-introduce-new-method-action}.
The occurrences of the method's body are then replaced with a
reference to the new action on
