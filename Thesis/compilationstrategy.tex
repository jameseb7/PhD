...
We explain these differences using the $HandleAstore$ action.
Recall that it is defined as shown below.
\begin{circusaction}
...
it is not guarded the value of $bc$ at the current $pc$ value.
The choice that the guards mediate is collapsed by Rule
[\nameref{HandleInstruction-refinement-rule}], so the guards are
removed from the $Handle{*}EPC$ actions.
...
$InterpreterAstore$, which does not affect $pc$, since the updates to
$pc$ are extracted from the $Handle{*}$ actions by Rule
