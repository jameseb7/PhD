...
This works because $HandleInstruction$ behaves as $\Chaos$ when $pc$
is outside the domain of $bc$.
In this rule we write $HandleInstruction$ with a $bc$ subscript to
indicate that it makes use of $bc$, which is a parameter of the $Thr$
pocess in which $HandleInstruction$ occurs. 
...
occurrence of $HandleInstruction$ at each $pc$ value in the loop at
line~\ref{algorithm-expand-bytecode-loop}.
Rule [\nameref{HandleInstruction-refinement-rule}], shown below, is
applied on line~\ref{algorithm-HandleInstruction-refinement} to refine
...
(e.g.\ $HandleDup$, $HandleAload$ etc.), which refer to as $Handle*$
actions.
They are named by appending $EPC$ to the names of the actions used to
define $HandleInstruction$, and we refer to them as $Handle*EPC$
actions.

The $Handle*EPC$ actions have some differences from the $Handle*$
actions on which they are based. 
Firstly, they are not guarded on the value of $bc$ at the current $pc$
value, since the choice those guards mediate is collapsed by Rule
[\nameref{HandleInstruction-refinement-rule}].
The parameters of the bytecode instructions are also transferred to
become parameters of the $Handle*EPC$ actions.
Finally, the $Handle*EPC$ actions do not affect $pc$, since the
updates to $pc$ are extracted from the actions by Rule
[\nameref{HandleInstruction-refinement-rule}].

% are obtained by first replacing
% $HandleInstruction$ with its definition, which is a choice between
% actions to handle each type of instruction (e.g.\ $HandleDup$,
% $HandleAload$ etc.).
% This choice is then collapsed to the action for handling the bytecode
% instruction at the given $pc$ value, $i$.
% That action is converted to a new action that has its Those new
% actions are not guarded on the value of $bc$ at the current $pc$
% value, since the choice those guards mediate has already been
% collapsed.

% a particular $pc$ value (e.g.\ $HandleDup$, $HandleAload$ etc.).
...
% appropriate $Handle^*$ actions on
% line~\ref{algorithm-collapse-handle-instruction}.
