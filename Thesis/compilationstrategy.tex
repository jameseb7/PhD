...
\begin{algorithm}
  \begin{algorithmic}[1]
    ...
    \State \Call{ApplyLemma\ref{if-introduction-lemma}}{$node$}
    \label{algorithm-introduce-if}
    \State \Call{ApplyLemma\ref{if-else-introduction-lemma}}{$node$}
    ...
    \State \Call{ApplyLemma\ref{loop-introduction-lemma}}{$node$}
    \label{algorithm-introduce-loop}
    \State \Call{ApplyLemma\ref{conditional-loop-introduction-lemma}}{$node$}
    \label{algorithm-introduce-conditional-loop}
    % \State \Call{ApplyLemma\ref{while-loop-introduction-lemma}}{$innerLoops$}
    % \label{algorithm-introduce-while-loop}
...
We require that the control flow graph be well-structured in order to
produce well-structured C code.
...
Taking these requirements into consideration, we follow the analysis
of program structure due to Dijkstra~\cite{dijkstra1972}.
This views structured programs as graphs composed of blocks, which are
themselves structured programs.
Thus the definition of structured program is recursive, with the
trivial case of a single node control flow graph forming the base
case.
The more complex structured control flow graphs have the forms shown
in Figure~\ref{structured-cfg-figures}.
\begin{figure}
  \begin{subfigure}{0.33\textwidth}
    \begin{center}
      \begin{tikzpicture}
        \useasboundingbox (-1,-1) rectangle (1,1.5);
        \node at (0,1)  (A) {$\bullet$};
        \node at (-1,0) (B) {$\bullet$};
        \node at (0,-1) (C) {$\bullet$};
        \draw[-latex] (A) -- (B);
...
nothing following the conditional.
...
Each of the nodes in the graphs shown in
Figure~\ref{structured-cfg-figures} may itself be another of those
graphs.
The node at the end of one of the graphs may be the node at the start
of another graph, representing the structures placed in sequential
composition with each other (we do not show simple cases of sequential
composition in the control flow graphs in this subsection, since they
will already have been removed by the algorithm in
Section~\ref{introduce-forward-sequence-subsection}).
There may instead be an edge connecting these nodes if the second
structure is a loop (since the looping edge prevents sequential
composition from being introduced between the two structures).
Similarly, a single node may be added before the beginning of a loop
structure.
For the nodes within loops, only graphs with a single start and end
node may take their place so that no additional exit points are added
to the loop.
This means that the structure shown in Figure~\ref{divergent-figure}
cannot occcur within loops.

The control flow graph in our example, shown in
Figure~\ref{example-simplified-control-flow-graph-figure}, may be
viewed as a \texttt{while} loop structure, with a node within the
replaced by an \texttt{if}-\texttt{else} conditional.
There is also an additional node at the beginning ($pc=7$) with an
edge leading to the start of the loop ($pc=39$).
Thus the \texttt{handleAsyncEvent()} method example is
well-structured according to the above definition.

Having defined the desired program structure in terms of a small
number of standard structures, the strategy can identify each of these
structures in the control flow graph and introduce them into the
program, collapsing the control flow graph in the process.
In order to identify the structures in isolation from other
...
the conditional. 
In the first case the introduction of the sequential compostion is
...
An \texttt{if} conditional with no else branch may be recognized from
the control flow graph as having the form shown in
Figure~\ref{if-figure}.
However, it can also be recognized from the form of the \Circus{} code
...
We provide Lemma~\ref{if-introduction-lemma} for introducing such
conditionals.
\begin{lem}[\texttt{if} conditional introduction]
  \label{if-introduction-lemma}
  ...
  \end{circus}
\end{lem}
Lemma~\ref{if-introduction-lemma} introduces a conditional for nodes
that match the form described above, which in the lemma is the
$pc = i$ node.
...
Lemma~\ref{if-introduction-lemma} is applied on
...
the lemma with a condition on the control flow graph.
...
Lemma~\ref{if-else-introduction-lemma}.
\begin{lem}[\texttt{if}-\texttt{else} conditional introduction]
  \label{if-else-introduction-lemma}
  \setlength{\zedindent}{0.25cm}
  % \setlength{\abovedisplayskip}{0.1cm}
  % \setlength{\belowdisplayskip}{0.1cm}
  If $i \neq j$, $i \neq k$, and 
  ...
  \end{circus}
\end{lem}
Lemma~\ref{if-else-introduction-lemma} operates similarly to
Lemma~\ref{if-introduction-lemma} in how it introduces the conditional
and moves the common $pc$ assignment outside the conditional.
However, Lemma~\ref{if-else-introduction-lemma} includes sequences of
...
conditional.
The preconditions of Lemma~\ref{if-else-introduction-lemma} are the
same as those of Lemmma~\ref{if-introduction-lemma}.
Lemma~\ref{if-else-introduction-lemma} is applied on
line~\ref{algorithm-introduce-if-else} of
Algorithm~\ref{introduce-loops-and-conditionals-algorithm}.

Once a conditional has been introduced, we can attempt to introduce
the sequential composition with the node after the conditional.
The requirement for this to occur is that the sequential composition
be a simple sequential composition from a node with a single outgoing
edge to a node with a single incoming edge, as in
Section~\ref{introduce-forward-sequence-subsection}.
Thus we check for a simple sequence on
line~\ref{algorithm-lci-sequence-check} of
Algorithm~\ref{loop-and-conditional-introduction-algorithm} and
introduce the sequential composition on
line~\ref{algorithm-lci-sequence-introduction}.
This also catches the case of a sequential composition before the
start of a loop, such as occurs at the $pc=7$ node in our example.
...
by applying Lemma~\ref{conditional-introduction-lemma} on
line~\ref{algorithm-introduce-conditional}.
This lemma introduces the conditional in much the same way as
Lemmas~\ref{if-introduction-lemma}
and~\ref{if-else-introduction-lemma}, but it does not place any
...
Lemma~\ref{while-introduction-lemma1}.
This lemma introduces a conditional at a node $pc=i$ with its false
...
Lemma~\ref{while-introduction-lemma2}, that introduces the loop in the
true branch of the conditional.
\begin{lem}[\texttt{while} loop introduction 1]
  \label{while-introduction-lemma1}
  \def\zedindent{0.25cm}
  If $i \neq j$,
  \begin{circus}
    \{frameStack \neq \emptyset\} \circseq A \\
...
% Conditionals can be identified as nodes whose sequence of instructions
% ends with an assignment of the form $pc := \IF b \THEN x \ELSE y$ and
% which have two target nodes, neither of which are earlier in the
% method (so as to distinguish them from loops).
