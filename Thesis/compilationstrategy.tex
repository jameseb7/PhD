...
We finish construction of the control graph for a method when we
encounter a return instruction.
...
Since there are finitely many nodes, the merging of nodes will result
in a state in which no further sequential compositions can be
...
constructs used to define structured control flow graphs, and merging
the nodes may be seen as performing the reverse of node replacement
for it.
\begin{figure}
  \begin{center}
    \begin{tikzpicture}[every new ->/.style={-latex}]
