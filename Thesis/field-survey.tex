...
The real-time threads of RTSJ run continously from when they are
...
Timers can also be used to trigger execution an asynchronous event
...
Many RTSJVMs appear to no longer be maintained, including Sun's Java
RTS.
We also note that, since SCJ is based on RTSJ, some RTSJVMs also
function as JVMs, and so we discuss them in more detail later.
...
(non-real-time) Java threads and the garbage-collected, RTSJ programs
are hard to verify to the level required for safety-critical
certifications.
SCJ thus restricts the features of RTSJ to better facilitate such
certification.
...
scheduler as in RTSJ, but SCJ only permits priority ceiling emulation
...
including JamaicaVM~\cite{aicas2017}, icecap
...
fact JamaicaVM is a very mature RTSJ implementation, it is used as the
basis for the SCJ reference implementation, and is thus also an SCJVM.
...
design goal.
It allows for efficient execution of Java bytecode programs while aslo
...
preceding discussion it appears to be widely applied, and there is a
lot of scope for error during the compilation, motivating the need for
formal verification techniques to ensure it is correct.
}
