...
% G(X) = X \circseq A
% H(X) = f(X)
\begin{law}[rec-seq-distr]
  \label{rec-seq-distr-law}
  If  $F(X \circseq A) \circseq A \circrefines_A F(X) \circseq A \circseq A$ then,
  \begin{circus}
    (\circmu X \circspot F(X) \circseq A)
    \circrefines_A
    (\circmu X \circspot F(X) \circseq A) \circseq A
  \end{circus}
\end{law}
