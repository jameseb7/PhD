...
After the presentation of our model, we discuss how it may be
validated in Section~\ref{cee-validation-section} and then conclude
with some final considerations in
...
\begin{table}[thp]
  \begin{center}
    \begin{tabular}{@{}llp{6cm}ll}
      ...
      *{}+{} & $initAperiodicEventHandler$ & \raggedright $ThreadID \cross ObjectID \cross Priority$\endgraf$\t1 {}\cross \nat \cross \nat \cross \nat$ & I & L \\
      *{}+{} & $initPeriodicEventHandler$ & \raggedright $ThreadID \cross ObjectID \cross Priority$\endgraf$\t1 {}\cross Time \cross Time \cross \nat \cross \nat \cross \nat$ & I & L \\
      *{}+{} & $initOneShotEventHandlerAbs$ & \raggedright $ThreadID \cross ObjectID \cross Priority$\endgraf$\t1 {}\cross Time \cross \nat \cross \nat \cross \nat$ & I & L \\
      *{}+{} & $initOneShotEventHandlerRel$ & \raggedright $ThreadID \cross ObjectID \cross Priority$\endgraf$\t1 {}\cross Time \cross \nat \cross \nat \cross \nat$ & I & L \\
...
The channels marked with a \dag{} represent calls to infrastructure
methods that are part of the SCJ API.
...
However,in the case of the methods for entering memory areas, the
channels omit the identifier of the \texttt{Runnable} object passed to
the methods, since the methods are implemented in Java bytecode using
the special methods provided by the channels.
This ensures the \texttt{run()} method of the \texttt{Runnable} object
can be executed in the interpreter.
The $exitMemory$ channel is used to implement a special method that
exits the memory area, which is used after the \texttt{run()} method
returns.

The channels marked with a {}+{} are special methods for implementing
the intialisers of the event handler objects.
They take as parameters the information passed to the constructors in
the SCJ API, but without the structure of the SCJ parameter classes.
The channels marked with a \S{} expose SCJVM scheduler operations to the
code executed in the interpreter. 
...
The second part contains information on the memory-area objects of the
program, including the relationship between the memory-areas and the
backing stores they represent, so that methods for entering and
...
The final part of the state contains information on alarms used for
coordinating releases of schedulable objects, so that they can be set
in the real-time clock in the correct order.
...
The $schedulableThreads$ map stores the thread identifier for each
schedulable object.
This map must be injective, since every schedulable object has a
separate thread.
The $schedulableTypes$ map stores information about which type of
...
The $schedulableSizes$ map stores the backing store, allocation area,
...
below.
It contains a map, $alarms$, that stores the time of the next alarm to
...
The state of the process is then the conjunction of these four
schemas.
\begin{circusaction}
...
$executeMethod$ and $executeMethodRet$, discussed
earlier.
...
\begin{circusaction}
  ...
  \t2 {} \extchoice InitPeriodicEventHandler \extchoice InitOneShotEventHandlerRel \extchoice InitOneShotEventHandlerAbs) \circseq \\
...
$HandleSpecialMethodsMainLoop$ handles memory-area entering methods,
after which another method is executed in the interpreter, during
which further special methods may be called.
...
special method is called in the $Interpreter$ exit the memory area. 
It's handling is specified by the $ExitMemory$ action below.
...
In addition to handling special methods during the execution of a
mission in $WaitForExecution$, the release of the event handlers of
the mission must also be managed by the $Launcher$.
This is performed by the $ExecuteThreads$ action, shown below, which
is executed in parallel interleaving with the action to handle special
methods.
In this interleaving, the state is partitioned such that the
$AlarmManager$, $SchedulableManager$ and the $perReleaseMemories$ map,
are controlled by the $ExecuteThreads$ action, with all other state
...
information is transferred to and from the $HandleAlarms$ action via
the channels $setAlarm$ and $triggerAlarm$, which are hidden.
A separate copy of $HandleThread$ is used to manage each possible
schedulable object, with the exception of the $clockHandler$ object,
...
interrupt thread via the $runThread$ channel.
When a request to set an alarm for a thread is accepted, the thread's
identifier, $thread$, and the time of the desired alarm, $alarmTime$,
are received via the $setAlarm$ channel.
These are then added to the $alarms$ map with $thread$ associated to
$alarmTime$, in the data operation $AddAlarm$.
Execution of the $clockHandler$ thread begins with receiving the
thread identifier, $tid$, an object identifier, which must be that of
the $clockHandler$ object, and a method identifier, which must be that
of the \texttt{handle()} method.
...
Each of the thread identifier's in the $threads$ set are then
...
one-shot event handler that was initialised with a relative release
time.
...
The action is guarded so that it will only be offered if $oseh$ is a
...
If such a backing store does exist, then we enter it and execute the
\texttt{handleAsyncEvent()} method of the object $oseh$ using the
$executeMethod$ channel.
...
%TODO: need better ending for this section
...
The final component, $stackSize$, stores the maximum size of the
...
\where
  ...
  \exists_1 m : MethodID | m \in \dom currentClass.methodEntry @ \\
...
Finally, the third conjunct states that $pc$ must be in the
$ProgramAddress$ range for a unique method of $currentClass$, to
ensure the bytecodes are associated with the correct class
information.
...
\begin{circusaction}
  ...
  \t1 Blocked \circseq runThread!thread!(head~methodArgs)!methodID \circseq Started
\end{circusaction}

The $Started$, shown below, offers a choice of three behaviours.
...
The interpreter can also end execution on the thread, signalled by the
$endThread$ channel, after which $Started$ finishes offering the main
choice and terminates the thread, as we describe below.
...
If $frameStack$ is empty then the method being returned from was
...
\begin{circusaction}
  HandleGetfield \circdef \lcircguard pc \in\dom bc \land bc~pc \in \ran getfield \land (getfield\inv)~(bc~pc) \in fieldRefIndices~currentClass \rcircguard \circguard \\
