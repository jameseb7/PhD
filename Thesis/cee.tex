...
\addchange{Added text to clarify what is meant by ``effective reasoning''}
\added{
In particular, each of the bytecode instructions included in our
subset must be handled by the compilation rules defined in
Chapter~\ref{strategy-chapter}.
More compilation rules would require more effort to define and prove,
but the additional compilation rules would be similar to those already
defined and so would add little to our strategy.
...
which are proved from laws of known correctness, as explained in
...
Since we also produce a prototype implementation of this compilation
strategy, described in Section~\ref{tool-support-section}, we can
easily produce the ouput of the strategy and compare it to the
corresponding code produced by icecap, indeed we do so for some
examples in Section~\ref{examples-section}.
This allows us to ensure that the C code produced is sensible and
hence, since the generated C code corresponds to the semantics of the
bytecode in the intepreter model, validates the semantics in our
interpreter model as correct.
}
