...
\texttt{PeriodicEventHandler}, and \texttt{OneShotEventHandler}.% , and
...
Within the $CheckSynchronisedReturn$ action, shown below, the
...
We do not apply synchronisation to static methods, following the
behaviour of icecap, since data guarded by synchronisation should be
stored in individual objects with a specific scope, rather than being
...
If the method is static or not synchronized, then no synchronisation
needs to be done and the action terminates.
\begin{circusaction}
...
The special-method actions check if the $static$ parameter is $\true$
(since we define them as static methods for simplicity of the
interface with the SCJVM), and diverges if it is not (since that
...
$methodArguments$ if the invoked method is recorded as synchronized
and not static in the class information.
...
\begin{circusaction}
  InvokeOther \circdef \\
  \t1 \circval classID : ClassID; \circval methodID : MethodID; \\
  \t2 \circval methodArgs : \seq Word; \circval static : \boolean \circspot \\
  ...
  \t2 CheckSynchronizedInvoke(class, methodID, methodArgs) \circseq \\
  \t2 \lschexpract InterpreterNewStackFrame \rschexpract \\
