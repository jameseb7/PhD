...
Sections~\ref{cee-struct-manager-section}
and~\ref{cee-c-program-section}.
...
a single parameter, $safeletClass$, which is the $ClassID$ of the
\texttt{Safelet} class.
...
procedure, which the launcher manages.
The order they are executed is determined by the dependencies between
class initialisers and classes, and is passed to the $Launcher$
process as a second parameter, $initOrder$, which is a sequence of
$ClassID$s.
\begin{circus}
...
The first part contains the identifiers of the objects that form part
of the SCJ mission model, so that the $Launcher$ can call methods of
those objects during SCJVM startup.
The second part contains information on the memory areas objects of
...
We proceed to describe the Z schemas specifying each part of the
state.
...
$MemoryAreaManager$ schema, shown below.
It contains the identifiers of the memory area objects for the
immortal memory, $immortalMemory$, and mission memory,
$missionMemory$.
There is a map, $backingStores$, that relates these object
identifiers, and the identifiers of the other memory area objects, to
the identifiers of the backing stores they represent.
...
The invariants of $MemoryAreaManager$ ensure that each memory area
object has a corresponding backing store in $backingStores$ and that
areas which are not nested private memories do not appear in the range
of $privateMemoryMap$.
Since the invariants are quite complex due to the need to describe the
relationships between all the different components, we do not include
them here.

The final part of the state is specified in the $SchedulableManager$
schema, shown below.
It simply contains a map, $schedulableThreads$, from the identifiers
of schedulable objects to the identifiers of the threads associated
with them.
...
schemas, combined into a $LauncherState$ schema as shown below.
\begin{circusaction}
...
The object identifiers ($safelet$, $missionSequencer$,
$currentMission$, $immortalMemory$ and $missionMemory$) are
initialised to the $null$ identifier.
They are later filled with non-$null$ identifiers as the corresponding
objects are created during SCJVM execution.
Similarly, each of the maps ($backingStores$, $perReleaseMemories$,
$privateMemoryMap$ and $schedulableThreads$) is initialised to the
empty set, to be filled as the items it records are created.
...
The main action of the $Interpreter$ then proceeds as shown below.
The state is first initialised as described by $LauncherInit$ and then
its behaviour is split into two actions $Startup$ and
$RunNextMission$, which occur in sequential composition.
The $Startup$ action represents the SCJVM startup procedure that must
only be performed once at the start of SCJVM execution, whereas
$RunNextMission$ is the procedure that must be performed for each
mission run.
We do not handle mission termination in the $Launcher$ model.
This is because SCJ's mission termination procedure is quite complex,
a formal account of it is already given
elsewhere~\cite{cavalcanti2013, luckcuck2016, zeyda2011}, the
termination procedure has almost no effect on our compilation
strategy, and a single mission is sufficient for our examples to test
the compilation strategy.
Thus, $RunNextMission$ is only executed once, whereas in a model that
did handle mission termination it would execute in a loop that
repeatedly performs $RunNextMission$ for each mission returned by the
mission sequencer.
\begin{circusaction}
...
\item calling the \texttt{immortalMemorySize()} and
  \texttt{globalBackingStoreSize()} of the $safelet$, and checking
  that the size of the root backing store matches those values, in
  $CheckImmortalMemory$ and $CheckRemainingBackingStore$;
\item calling the the \text{initializeApplication()} method of the $safelet$, in $InitializeApplication$;
...
The procedure for running a mission is then defined in
$RunNextMission$, shown below.
This begins with calling the \texttt{getNextMission()} method of
$missionSequencer$, in the $GetNextMission$ action.
...
$ResizeMissionMemory$.
Next, in $InitializeMission$, mission's \texttt{initialize()} method
is executed, during which the schedulable objects for the mission are
registered.
Then, in $InitialiseAndStartThreads$, the registered schedulable
objects have their stacks and backing stores created, after which the
threads for all the schedulable objects are started.
Finally, in $WaitForTermination$, the $main$ thread suspends itself
and the $Launcher$ then waits, handling special methods when necessary
for the threads of the program.
Since termination is not actually handled, this phase of the program
continues indefinitely.
\begin{circusaction}
  RunNextMission \circdef GetNextMission \circseq ResizeMissionMemory \circseq InitializeMission \circseq \\
  \t1 InitialiseAndStartThreads \circseq WaitForTermination 
\end{circusaction}
...
earlier.
The identifiers of these methods, which may be standard methods from
the SCJ API or implementation-defined API methods required by the
launcher, are represented by constants in the model.
Although most of the methods used by the Launcher are executed simply
by communicating on each of the channels mentioned above in turn, the
\texttt{initialize()} method of a mission needs handling of the
\texttt{register()} method for each schedulable object, and other
special methods are defined at the same time.
These special methods are offered in external choice with the
communication on the $executeMethodRet$ channel during execution of
the \texttt{initialize()} method.
In the $WaitForTermination$ action the special methods are offered on
their own without offering communication on $executeMethodRet$.

The special methods for entering memory areas require extra handling,
since another method is executed once the memory area has been
entered.
The $ExecuteInAreaOf$ action, shown below, is the simplest of the
actions for handling memory area entering methods, so we use it here
to illustrate the system for handling such methods.
The memory areas entered are tracked separately for each thread, so
this action takes a thread identifier parameter, $thread$, and only
accepts channel communications on that thread.
...
The class of the $runnable$ object is then determined and its
...
\begin{circusaction}
  ...
  \t1 \circval thread : ThreadID \circspot \circvar bs : BackingStoreID; runnable, object : ObjectID \circspot \\
...
memory area, is specified by the $ExitMemory$ action shown below.
...
The $Launcher$ state may then be updated to account for the exited
memory area being cleared (due to no longer being in use by any
thread), which is specified in the $ClearPrivateMemory$ schema and
which we omit here since it is not the focus of this discussion.
...
The entries to and exits from memory areas are counted for each thread
so that each entry is matched by one exit and $ExitMemory$ is only
offered for threads that have entered at least one memory area through
the memory entering actions.
This is ensured by the definition of the
$HandleSpecialMethodsMainLoop$ action, part of which is shown below. 
It uses a map, $memoryEntries$, to record how many memory areas have
been entered for each thread, incrementing it after each memory entry
(described by $EnterMemory$, which combines the memory entering
operations) and decrementing it after each memory exit (described by
$ExitMemory$).
The choice of actions is offered separately for each thread so that we
know which thread the entry or exit is occurring on.
The action recurses after an entry or exit so that further special
...
Apart from the memory area entering methods, the \texttt{register()}
method also needs to execute a method in the interpreter to obtain a
thread's priority.
Further special methods are not handled in this case, since the method
is expected to be a simple method that does not call special methods
and so further complicating matters is unnecessary.

In order to allow the special methods to be handled, the interpreter
(or C code) must communicate with the $Launcher$ when such methods are
encountered.
...
be prepared to execute a method after signalling to the $Launcher$.
This then gets transformed to nested method executions during the
compilation strategy in order to preserve the communication pattern
with the launcher.
We describe how this communication is performed in the interpreter and
C code in more detail in later sections.
...
\label{cee-struct-manager-section}
