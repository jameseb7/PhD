...
For those that use a bytecode interpreter, the components are listed
below and shown in Figure~\ref{cee-fig}:
\begin{itemize}
...
compilation (i.e.\ Figure~\ref{cee-model-fig} holds after compilation,
except for the renaming of the components), but different channels are
used for communication between the interpreter and object manager
after compilation.
The other channels used for communication between the CEE components
remain unaffected by the compilation.
...
first two columns of the table, we also indicate which components of
the CEE make use of the channel, in the final two columns.
In those columns the names of the components are abbreviated due to
space constraints:~L refers to the launcher, I refers to the
Interpreter, and OM refers to the object manager.
...
As we do not nodel the console device itself, these are left as
...
Most of the channels occur in pairs of channels:~one channel to
communicate a signal to begin an operation and supply any inputs to
the operation, and a return channel to communicate back when the
operation has finished and supply any outputs of the operation.
...
channel.
We will discuss the reason for each channels deviation from this
pattern as we discuss the channels themselves.
...
The interpreter returns on $executeMethodRet$ channel when it has
...
communicated to the interprete.
This is handled by the $initMainThread$ channel, which carries the
$StackID$ for the stack space allocated for the $main$ thread.
This does not require a return channel as there is no return value.
Also, the interpreter will wait for communication on the
$executeMethod$ channel before commencing execution, so the launcher
does not need to wait for the interpreter to finish registering the
$main$ thread's stack.
...
channels associated with them) are taken from the SCJ specfication.
Among these, the ones also marked with a \dag{} are methods that do
not return a value and involve execution of a method in the
interpreter as part of their handling.
...
return channels.
The channels marked with a + are exposing SCJVM scheduler operations
to the code executed in the intepreter, in order to allow for the
...
given thread, communicating the $ThreadID$ of that thread.
...
FInally, channels used by the interpreter are the $addThreadMemory$
...
the launcher, which is shared between the bytecode interpreter model
and the C code model.
