...
The communications with the SCJVM services memory manager and
scheduler use the channels already described in
...
After the compilation, most of these channels remain in use but are
taken as representing C function calls.
The channels used for accessing fields are replaced with channels to
obtain a struct representing the contents of an object and to store an
object's struct after updating it.
The $getClassIDOf$ channel is similarly replaced with an access to a
field of the struct storing the $ClassID$ of the class of the object.
Note that, since the launcher is unaffected by the compilation
strategy, we retain the use of the $getClassIDOf$ channel for the
launcher as a loose specification that the launcher should get the
$ClassID$ of an object.
In an implementation the launcher may be refined to use C struct
accesses that are the same as those used in the C code.
