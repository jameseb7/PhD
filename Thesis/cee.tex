...
$Bytecode$ values.
The $bc$ map and the $cs$ map described in
Section~\ref{cee-classes-subsection} are passed as parameters to the
$Interpreter$ process, as can be seen from its definition shown below.
...
The start definition of the $Thr$ process is shown below.
This accepts the $bc$ and $cs$ parameters from $Interpreter$, plus the
$ThreadID$ of the thread $Thr$ represents, $thread$.
\begin{circus}
  \circprocess Thr \circdef \\
  \t1 bc : ProgramAddress \pfun Bytecode; cs : ClassID \pfun Class; thread : ThreadID \circspot \\
  \t1 \circbegin
\end{circus}

The state of $Thr$ contains the stack for the thread, which consists
of a series of stack frames, one for each method on the call stack.
...
The state of $Thr$ is given by the schema $InterpreterState$, shown
below.
Its first component, $frameStackID$, is the identifier of the stack
that serves as the frame stack for the thread. 
...
entered.
The third component, $pc$, is the program counter value for the
thread.
Finally, the fourth component, $currentClass$, is a copy of the class
information for the current method, so that it can be easily
referenced in the body of $Thr$.
...
\end{schema}
The first conjunct of the invariant of $InterpreterState$ requires
that there be no $StackFrames$ on the $frameStack$ when $frameStackID$
is $Uninitialised$.
The second conjunct defines $currentClass$ as the same as the
...
The state is initialised as described in the schema $InterpreterInit$
below.
The initialisation simply sets $frameStackID$ to $Unintialised$ and
the $frameStack$ to empty.
...
\begin{schema}{InterpreterInit}
	InterpreterState~'
\where
	frameStackID' = Uninitialised \\
	frameStack' = \langle\rangle
\end{schema}

The main action of $Thr$ is shown below. 
It consists of initialisation as described in $InterpreterInit$,
followed by a choice of two different actions, $MainThread$ and
$NotStarted$, depending on whether the thread represented by the $Thr$
process is the $main$ thread or not.
...
The $MainThread$ action, which describes the behaviour of the $main$
thread's process, is shown below.
It begins by accepting a $StackID$ from the $Launcher$ on the
$initMainThread$, which is used to initialise the $frameStackID$ state
component.
...
The $StartIntepreter$ used by $MainThread$ is also handling special
methods that execute nested methods.
...
was created in reponse to an external request.
\begin{circusaction}
...
For threads that are not the $main$ thread, their behaviour is
described by the $NotStarted$ action shown below.
It accepts a request to start the $thread$ represented by the process
from the scheduler on the $CEEstartThread$ channel.
The backing store identifier, $bsid$, of the thread's backing store is
then passed to $ObjMan$ via the $addThreadMemory$ channel and the
$stack$ identifier is used to initialise $frameStackID$.
The remaining information is stored in $classID$, $methodID$ and
$methodArgs$, and the $Class$ information is determined and a new
$StackFrame$ is created, similarly to $StartInterpreter$. 
...
is signalled on the $CEEremoveThread$ channel to indicated that the
thread has finished.
This then causes the scheduler to switch to a different thread, so a
thread switch away from $thread$ is accepted on the $CEEswitchThread$
channel.
...
The $Blocked$ and $Running$ actions form the behaviour of threads
...
\begin{circusaction}
  ...
  \t1 \circfi
\end{circusaction}
The behaviour of polling for thread switches in $Poll$ is required in
order to permit thread switches inbetween bytecode instructions.
Note that implementations of the model that allow thread switches at
other points would be valid if they retain the same sequence of
externally visible events, meaning only instructions involving
communication with other parts of the model need be atomic.
...
Since its contents are not directly used in the compilation strategy,
we omit the definition of $Poll$ here.

The $HandleInstruction$ action, shown in part below, offers a choice
of actions for handling the bytecode instructions in our subset.
There is one action for each of the bytecode instructions, with the
action's name formed from the bytecode mnemonic prefixed with $Handle$
(e.g.\ $HandleAload$ for the \texttt{aload} instruction).
These actions provide the semantics for the instructions and, as such
are heavily used in the compilation strategy.
\begin{circusaction}
...
similar form. 
The simplest of these consist of a guard requiring the $bc$ value at
the current $pc$ to be a particular bytecode instruction, followed by
a data operation specified by a Z schema updating $InterpreterState$.
This can be seen in the definition of $HandleAconst\_null$ below,
...
The $HandleDup$, $HandleIadd$ and $HandleIneg$ actions also follow the
simple form above.
...
This can be seen in the $HandleAload$ action, shown below, in which
the inverse of the $aload$ constructor is used to extract its
parameter into a $variableIndex$ variable that is used by the
$IntepreterAload$ schema.
\begin{circusaction}
...
value to the $Launcher$ when returning from a method that was started
by the $Launcher$.
...
shown below, the boolean value $fromBaseFrame$, passed as a parameter
to the action, is used to determine whether the return value should be
communicated to the $Launcher$.
If $fromBaseFrame$ is set to $\true$, then that indicates the return
is from a frame created by an external request, which normally
indicates that the return value needs to be passed to the launcher.
...
If the condition is not true then the action terminates without
communicating with the $Launcher$.
\begin{circusaction}
...
\texttt{invokestatic} and \texttt{invokevirtual}), require more
complex handling due to the fact that certain methods require special
...
This can be seen in the definition of the $HandleInvokespecial$
action, shown below.
...
the method (which is not included in the arguments encoded in the
method identifier).
...
similar, except $argsToPop?$ does not include the extra \texttt{this}
argument in $HandleInvokeStatic$, and $HandleInvokevirtual$ obtains
the class identifier from the \texttt{this} object identifier via the
...
channels.
This can be seen in the $InvokeResumeThread$ action, shown below.
...
Some of the special methods start new methods as part of their
execution, which requires additional handling.
An example of such a method is the \texttt{enterPrivateMemory()}
method of the \texttt{ManagedMemory} class from the SCJ API, which
...
the SCJVM.
This can be seen in the $InvokeRead$ action below, which accepts the
input on the $input$ channel and pushes it onto the stack as the
...
It begins with a guard that is the negation of the guards for the
invocation actions for the special methods.
