...
identifier of the \texttt{Runnable} object passed to the methods is
not communicated on the channels used to implement those methods.
In response to a signal on those channels, the launcher instead simply
enters the appropriate memory area, and the \texttt{Runnable} object's
\texttt{run()} method is called separately by executing a bytecode
instruction in the interpreter.
Another special method is provided to exit memory areas after the
...
of objects of classes that group together information such as the
storage sizes and scheduling times.
For simplicity, the channels used for the event handler intialisers
just accept the information directly, without the added structure of
these objects.
