...
Note that we only provide constructors for
\texttt{AperiodicEventHandler}, \texttt{PeriodicEventHandler}, and
\texttt{OneShotEventHandler}, and we do not include the forms of the
constructors that allow for a deadline miss handler to be set.
This is because the differing types of event handlers do not affect
the compilation strategy, so it is only necessary to implement the
event handlers needed to evaluate the strategy.
The infrastructure for the event handlers is contained in the
Launcher, which is not affected by the compilation strategy. 
A formal account of the execution model for the event handlers is
available elsewhere~\cite{cavalcanti2013, luckcuck2016, zeyda2011}.
...
This model could be extended to include deadline miss handlers,
\texttt{AperiodicLongEventHandlers} (which take an additional
parameter when they are released), and features from SCJ level 2
without affecting the correctness of the compilation strategy.
However, since a formal account of those features already
exists~\cite{cavalcanti2013, luckcuck2016, zeyda2011}, we include in
our model only those features necessary to support evaluation of the
compilation strategy.
...
parameter and checked in a data operation $CheckSuperclass$, which
replaces it with the identifier of the direct superclass of
$currentClass$ if it is a proper superclass of $currentClass$, in line
with the semantics specified for the JVM. 
...
with $poppedArgs$, and a boolean value indicating that the
method invocation is not from the \texttt{invokestatic} instruction.
\begin{circusaction}
