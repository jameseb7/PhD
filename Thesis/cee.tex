...
\texttt{run()} method of the \texttt{Runnable} object must be executed
after the memory area has been entered.
To allow this to occur, the methods are implemented in Java bytecode,
and the channels associated with them implement helper methods that
simply enter the appropriate memory area.
Since these do not use the \texttt{Runable} object, its identifier is
not communicated on their channels.
A special method is also provided to exit memory areas after the
\texttt{run()} method returns.
It is implemented using the $exitMemory$ channel.
...
the SCJ API.
However, the event handler constructors accept parameters in the form
of parameter classes whose constructors are provided with the
information to be passed to the event handler constructors.
For simplicity, the channels representing the event handler
intialisers simply accept the information directly.
A constructor written in Java must extract the information to pass it
to the initialiser methods in our SCJVM model.
...
$Started$ is replaced with a use of $ExecuteMethod$ in the same way as
for the same sequential composition in $MainThread$.
