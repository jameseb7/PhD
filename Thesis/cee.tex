...
% Additional constraints on the $bc$, $sid$ and $initOrder$ parameters
% to $CEE$ are also required to ensure complete divergence freedom.
% These are supplied as constraints on the corresponding constants in
% Appendix~\ref{constants-appendix}.
...
The creation of class objects in $MakeClassObjects$ requires some
...
involving $fields'$, while $\Xi Class \hide (fields)$ ensures the
...
performs the communication with the memory manager, which we discuss
below, after the definition of $MakeClassObjects$.
...
The second parameter is the thread in whose allocation context the
object is to be allocated, which in this case is the $main$ thread,
since that is only thread that is initially eligible to run.
...
the class information for the object, and $thread$, representing
...
$ObjMan$ enters offers services to the other components of the $CEE$,
in the $Loop$ action shown below.
The services offered by $Loop$ include $NewObject$, which offers the
service of creating an object as described in $MakeObject$.
...
$GetClassIDOf$ offers the service of obtaining the $ClassID$ for the
class of an object, by extracting the $this$ identifier from the
...
Finally, the $GetCurrentAC$ action offers the service of obtaining the
current allocation context for a given thread, which is the backing
store on top of its stack in $backingStoreStacks$.
\begin{circusaction}
...
($MakeObject$) are affected compilation strategy.
$GetClassIDOf$ is also affected since an object's class identifier is
stored as part of its structure.
The management of allocation contexts is
unaffected by the compilation strategy, but is required for managing
allocation of objects, so that they can be allocated in the correct
backing store.
...
and $newObjectRet$ channels, and the $GetClassIDOf$ is simply
extracting the $this$ identifier from the $Class$ information of an
$object$.
...
The $bc$ and $cs$ values are passed as parameters to the $Interpreter$
