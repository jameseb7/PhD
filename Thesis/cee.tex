...
turn, the \texttt{initialize()} method of a mission requires handling
of the \texttt{register()} method for each schedulable object.
...
\end{circusaction}
The complexity in the $HandleSpecialMethodsMainLoop$ action comes from
the handling of memory-area entering methods, which execute another
method after entering a memory-area, during which further special
...
map, which records how many memory-area entries have occurred on each
thread.
...
To illustrate how the execution of another method is performed when a
memory-area is entered, we show the $ExecuteInAreaOf$ action below,
which is one of the actions offered in external choice in
$EnterMemory$.
