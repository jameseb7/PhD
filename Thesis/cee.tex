...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}
    ...
    \added{\node[right=5cm,align=center] (RTC) {Real-time\\Clock};}
    \node[] (S) {Scheduler};
...
threads should be started or suspended during mission
execution\addchange{Added mention of the launcher's communication with
  the real-time clock}\added{, and with the real-time clock to set
  alarms coordinating event-handler execution}.
...
summarised in \deleted{Figure}\added{Table}~\ref{cee-channel-table},
with the full channel declarations shown in Appendix~B of the extended
version of this thesis~\cite{baxter2018-extended}.
In addition to presenting the name and type for each channel, in the
first two columns of the table\deleted{.}\added{,} we also indicate
which components of the CEE make use of the channel\added{, in the
...
\begin{table}[thp]
  \begin{center}
    \begin{tabular}{@{}llp{6cm}ll}
      ...
      & \added{$runThread$} & \added{$ThreadID \cross ObjectID \cross MethodID$} & \added{I} & \added{L} \\
      *\dag & $register$ & $ThreadID \cross ObjectID$ & I & L \\
      \added{*\dag} & \added{$releaseAperiodic$} & \added{$ObjectID$} & \added{I} & \added{L} \\
...
the $CEEstartThread$\deleted{$Channel$}\added{ channel}.

\addchange{Added $runThread$ channel to discussion in CEE overview}
\added{%
When a thread other than the main thread starts running, the
$Interpreter$ is signalled by the scheduler and begins executing the
thread. 
However, the entry points of most threads are handled in the
$Launcher$, since event-handler threads, for example, form part of the
SCJ infrastructure.
A channel, $runThread$, is thus used to pass execution from the
$Interpreter$ to the $Launcher$ after initial setup of the thread has
been completed.
This carries the $ThreadID$ of the thread, along with the identifier
of the object representing the thread (either an event handler or an
interrupt handler) and the identifier of the method to be executed
(either \texttt{run()} or \texttt{handle()}).
The $Launcher$ then handles the execution of the thread, calling back
to the $Interpreter$ using the $executeMethod$ channel when necessary
to execute programmer-supplied code in the handler.
}
...
It carries the $ObjectID$ of the object whose field is to be
accessed\added{, the $ClassID$ of the object's class} and the
$FieldID$ of the field to be accessed.
...
to the $ObjectID$\added{, $ClassID$} and $FieldID$ that identify the
object and field to update.
...
$ClassID$ values rather than $ObjectID$ values, since static fields
are attached to classes rather than objects.
...
The complete definition can be found in Appendix~B of the extended
version of this thesis~\cite{baxter2018-extended}.
...
\addchange{Removed elision of $MemoryAreaManager$ invariant}%
\changed{\begin{schema}{MemoryAreaManager}
...
\where
  ...
  (backingStores\limg \{ immortalMemory, missionMemory \} \rimg \\
  \t1 {} \cup \ran perReleaseMemories) \cap \ran privateMemoryMap = \{\}
\end{schema}}%
...
The first of these is specified by taking the transitive closure of
$privateMemoryMap$ to obtain the mapping from backing stores to
backing stores that are (directly or indirectly) nested within them.
This is then required to be disjoint from the identity map from a
backing store to itself.
The second of these properties is specified in a similar way, stating
that the backing stores corresponding to $immortalMemory$,
$missionMemory$ and the $perReleaseMemories$ must be disjoint from
those in the range of $privateMemoryMap$.
}
...
\addchange{Removed elision of $SchedulableManager$ invariant}%
\changed{%
...
turn\added{,} in $InitializeMission$\deleted{,} the
...
(handled in the actions $Register$,\deleted{ $Suspend$,
  $Resume$}\added{ $TakeLock$, $ReleaseLock$}, etc.),
...
\begin{circusaction}
  ...
  \t1 \circfi \circseq exitMemoryRet \then \Skip
\end{circusaction}
...
interpreter in Section~\ref{cee-interpreter-subsection}, and in\added{
  the} C program in Section~\ref{cee-c-program-subsection}.
...
The $clockHandler$ thread execution\deleted{ then} is then signalled
to finish via the $endThread$ channel so that the interrupt handler
can execute again.
\begin{circusaction}
...
alarm to set, then $HandleAlarms$ recurses to offer the choice of
actions again.
...
$exitBackingStore$\added{ channel} and the end of the thread's
execution is signalled via the $endThread$ channel, since there are no
further releases of the event handler.
...
added\deleted{, but small enough to permit effective reasoning}.
\addchange{Added text to clarify why a subset has been used}%
\added{%
Additional instructions would be similar to those already defined, and
so would add little value to establishing the scientific basis of our
work.
The semantics of any additional instructions and the compilation rules
required for them can be obtained from the semantics of the
instructions in our subset and the compilation rules defined in
Chapter~\ref{strategy-chapter}.
Indeed, our prototype implementation of our compilation strategy,
described in Section~\ref{tool-support-section}, implements some
additional instructions to support the examples described in
Section~\ref{examples-section}.
For example, many Java bytecode instructions differ only in the types
they operate over, so such instructions are handled in the same way by
...
\afterpage{\centering
  ...
  \begin{supertabular}{llp{7.2cm}}
    ...
    \\
    \texttt{goto} & program address\added{ offset} & Unconditionally branches
    to the given program address.
    \\
    ...
    \\
    \texttt{if\_icmple} & program address\added{ offset} & Pops two integer values
    ...
    specified method of the class of\added{ the} referenced object.
    \\
...
handled.
\addchange{Remove statement about how \texttt{long} values in SCJ API
  methods are handled, since they are handled normally in the
  prototype}%
\deleted{%
...
tables to implement method lookup so the semantics of the
...
the memory manager, must be atomic since\deleted{ that} they affect shared
state.
...
% \end{schema}
The entries of $constantPool$ are indexed by\deleted{ a} elements of a
type $CPIndex$.
...
we omit them here.
We recall that the definitions omitted here are given in Appendix~B of
the extended version of this thesis~\cite{baxter2018-extended}.
...
a\added{n} object of a Java class is assignable to a variable of a given class.
...
This has an additional invariant, relating the parts together\deleted{.
The first conjunct of the invariant}\added{, which} requires that the domain of
...
The identifier of the new object is stored in $objectID$.
...
The full model of the object manager can be found in Appendix~B of the
...
\addchange{Removed elision of part of the definition of $Bytecode$}%
...
by the free type $Bytecode$,\deleted{ sketched}\added{ shown} below.
...
constructor.
\deleted{The full definitions omitted here are in Appendix~B of the extended
version of this thesis~\cite{baxter2018-extended}.}%
\changed{\setlength{\zedindent}{0.5\zedindent}%
\begin{zed}
...
\end{schema}
The invariant of $StackFrame$ just requires the $operandStack$ to\deleted{ not}
be\added{ no} larger than $stackSize$.
...
The\deleted{ $StartIntepreter$}\added{ $StartInterpreter$} action,
used by $MainThread$, handles requests to execute a method from the
$Launcher$.
...
signalled on the $SendThread$ channel\addchange{Added an error report
  communication to the \protect\texttt{endThread} scheduler
  operation}\added{, with a report of $Sokay$ expected in response}.
...
\changed{\setlength{\zedtab}{0.4cm}
\begin{circusaction}
  Started \circdef \\
  \t1 \circblockbegin
...
such\added{,} are involved in the compilation strategy.
...
They can be found in Appendix~B of the extended version of this
...
\begin{table}[ht]
  ...
  \begin{tabular}{llp{5cm}}
    ...
    \texttt{getfield} & $HandleGetfield$  & \added{$InterpreterPop$, }$InterpreterPush$ \\
...
of the current $StackFrame$ using the data operation\deleted{
  $IntepreterPop$}\added{ $InterpreterPop$}, and the class identifier,
$cid$, and the field identifier, $fid$ are extracted from the
parameter to the bytecode instruction.
...
the method.
The $cid$ and $mid$ identifiers are the\added{n} passed into $Invoke$
along with $poppedArgs$.
\begin{circusaction}
...
\begin{circusaction}
  ...
  \t1 {} \extchoice InvokeReleaseAperiodic(classID, method, args) \\
...
\begin{circusaction}
  InvokeRead \circdef \\
  \t1 \circval classID : ClassID; \circval method : MethodID : \seq Word \circspot \\
...
method identifier and method arguments as inputs, and performs its
...
\begin{circusaction}
  ...
  \t3 {} \lor methodID = releaseAperiodicID) \\
...
replaced with a struct manager and the interpreter is replaced with the
C program.
...
as shown in the definit\added{i}on of $CProg_{bc,cs}$ below.
\begin{circus}
...
return values.
We follow this representation rather than that of of icecap, since
icecap passes values using a stack represented by a pointer passed to
each function.
That approach is used in icecap to provide for interaction between
interpreted and compiled code, which we do not require in our code.
...
\addchange{Added explanation of types used in our C code}
\added{
...
\addchange{Added explanation of \texttt{uintptr\protect\_t} cast in
  struct accesses}
\added{
Note that our C code for accesses to structs involves a cast from
\texttt{int32\_t} to a pointer type.
In order to ensure this cast is performed correctly on systems where
pointers are not 32-bit, we also perform a cast to
\texttt{uintptr\_t}, the unsigned integer type from C99 with the same
width as a pointer.
This matches icecap, where a cast to a \texttt{pointer} type, defined
in the same way as \texttt{uintptr\_t} is performed as part of struct
accesses.
}
...
We omit the definitions of these actions here; their definitions can
be found in Appendix~B of the extended version of this
thesis~\cite{baxter2018-extended}, where we show the general form of
the $StructMan_{cs}$ process.
...
\texttt{Object} class, which is always va\deleted{i}l\added{i}d since
every class extends \texttt{Object}, and an access of the $class$
field of that struct type.
...
developing the SCJ specification, who\deleted{ has}\added{ have} been
able to clarify several points about SCJ.
...
instance\added{ of the} class implementing \texttt{Safelet} requires
initialisation, although the JVM specification states that every
object must be initialised.
...
\addchange{Added a more detailed description of how the compilation
  strategy validates the model}%
\deleted{
...
\added{%
Our compilation strategy consists of individual compilation rules,
which are proved from laws whose correctness has been previously
established, as explained in Section~\ref{proofs-of-laws-section}.
We can thus have confidence that the semantics of the C code that
results from the strategy has the same semantics as the bytecode input
to our interpreter model.
In addition, since we also produce a prototype implementation of this
compilation strategy, described in Section~\ref{tool-support-section},
we can easily produce the output of the strategy and compare it to the
corresponding code produced by icecap; indeed we do so for some
examples in Section~\ref{examples-section}.
Since the generated C code corresponds to the semantics of the
bytecode in the interpreter model, this validates the semantics in our
interpreter model.
}
% explain that the transformations of the strategy are provably
% correct (reference proofs-of-laws-section.
% Then, mention automatic translation tool (reference
% tool-support-section), which produces correct output that
% corresponds to the semantics of the interpreter model.
% After, that mention examples (reference examples-section) and the
% fact that we compare the generated code to the code produced by
% icecap, thus establishing that the generated C code is as-expected.
% And, since we have constructed a correct strategy and the C code is
% generated by a tool that closely follows that strategy, it follows
% that the interpreter model is correct.
% And since the C code model has the same semantics as the intepreter
% model (due to being the result of the compilation strategy), it is
% also correct.
...
Thread switches must not occur in the middle of these
instr\added{u}c\deleted{u}tions to avoid leaving shared memory in an
erroneous state.
