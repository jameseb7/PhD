...
As already said, the SCJVM memory manager is responsible for managing
the backing stores that underlie memory areas, including creation,
clearing, and resizing of backing stores, and allocation within them.
...
layers, beginning with memory blocks that allow allocation, then
adding in the structure of backing stores that may contain other
backing stores nested inside.
...
We have specified the real-time clock that tracks the current time and
...
are largely expressed in Z allows them to be checked and proved using
Z proof tools.
