...
\begin{figure}[ht]
  \centering
  \begin{tikzpicture}
        \node[draw,align=center] (MM)  at (7cm,2cm) {Memory\\Manager};
        \node[draw,align=center] (S)   at (1.5cm,1.5cm)  {Scheduler};
        \node[draw,align=center] (RTC) at (9cm,0cm)  {Real-Time\\Clock};
        \draw (0cm,-2.1cm) rectangle (10.5cm,3cm);
        \node at (1.5cm,2.7cm) {SCJVM Services};
        
        \node[align=center] (HW) at (2.5cm,-3cm) {Hardware};
        
        \draw[->] (RTC) edge node {$RTCclockInterrupt$} (S);
        
        \draw[-] (HW) -- node[right] {$HWinterrupt$} coordinate[pos=1] (X) ++(0cm,2.5cm);
        \draw[->] (X) to (S);
        \draw[->] (X) to (RTC);
        
        \node[align=center] (CEE) at (-2cm,1.5cm) {Core\\Execution\\Environment};
        \draw[<->] (S) -- (CEE);
        
        \node[align=center] (App) at (5.25cm,4cm) {SCJ Application\\and API};
        \draw[<->] (5.25cm,3cm) -- (App);
      \end{tikzpicture}
