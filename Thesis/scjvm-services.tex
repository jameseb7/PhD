...
the model.
As an example of this, consider that the memory manager model consists
...
The reason these files are smaller than our memory manager model
is that our memory manager contains invariants specifying the
...
stores.
Note that since \texttt{vm.Memory} from icecap represents a single
backing store, icecap does not require a class analogous to
$GlobalMemoryManager$, since each memory area can reference its
backing store directly without looking it up in a global map from
backing store identifiers to backing stores.
However, the inclusion of the $GlobalMemoryManager$ in our model
allows us to specify the global structure of backing stores and ensure
that each of the operations preserve that structure.
