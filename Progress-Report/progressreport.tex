...
requirements for an SCJVM is one component of our proposed framework
for verification of SCJVMs.
Our work so far has focused on the specification of requirements since
it must be completed before work on the compilation strategy can begin
as it is needed as a starting point for the compilation strategy and a
specification of the services available to the compiled code.
...
\begin{itemize}
\item the core execution enviroment, which manages execution of
...
\end{itemize}
This division of the SCJVM into two parts may be seen in
...
We created a specification of the SCJVM services first as the core
execution environment needs to make use of them.
...
deciding what low-level virtual machine services were necessary to
support them.
The services were divided into three areas, as can be seen in
Figure~\ref{scjvm-fig}, based on the chapters of the SCJ standard the
services they support belong to:
\begin{itemize}
\item the memory manager contains the services supporting the `Memory
  Management' chapter,
\item the scheduler contains the services supporting the `Concurrency
  and Scheduling Models' chapter and also the services for managing
  the interrupts that must be handled by the scheduler described in
  the `Interaction with Devices and External Events' chapter, and
\item the real-time clock contains the services supporting the
  `Clocks, Timers, and Time' chapter.
\end{itemize}
We decided not to handle further parts of the `Interaction with
Devices and External Events' chapter until the hardware interaction
model was finalised.
Our identification of the required SCJVM services and their behaviour
was checked with a member of the SCJ committee.

We created a formal model of the SCJVM services using the \Circus{}
...
The formal model of the memory manager has been partially proved using
Z/Eves.
...
covering memory blocks and backing stores have been proved.
A few proofs in the global memory manager section have also been done.
Additionally, many theorems about objects in the Z/Eves mathematical
toolkit have been proved in the course of proving the model.
...
There are 13 memory manager services, 20 scheduler services and 4
...
We have also collaborated in some related work creating a \Circus{}
model of the icecap scheduler, which is expected to be a refinement of
our scheduler.
A paper describing this related work has been submitted to the 12th
...
However, our subset must be large enough to express a wide variety of
SCJ programs and to illustrate how it may be extended with further
features.
...
This was so as not to unnecessarily complicate the model with trivial
or redundant instructions and better allow ourselves to concentrate on
the instructions that would be most of interest in creating the
compilation strategy.
...
However, features of SCJ such as disallowing dynamic class loading
allowed for simplifications to the semantics of some instructions,
such as simplifying method lookup with precomputed method tables.
...
However, it was realised that, in order to properly specify the
semantics of some instructions for creation of objects and invocation
of some special methods and ensure that the semantics of the core
execution environment as a whole was specified, it would be necessary
to model the SCJ startup process and mission model.
We did this as a separate \Circus{} process called the launcher, which
is responsible for coordinating the execution of bytecode instructions
by the interpreter and giving special handling to infrastructure
methods.
The separation of the core execution enviroment into the launcher and
interpreter also necessitated separating out of state required by both
of those processes.
To that end we separated out the class information and provision of
bytecode instructions (which both do not change after the SCJVM
starts) to a process called the code area, and handling of objects to
a process called memory.
So the core execution environment is split into four areas, as seen in
Figure~\ref{scjvm-fig}, each represented by a \Circus{} process:~the
code area, the memory, the interpreter and the launcher.
...
introduce special method identifiers and auxillary functions on given
types representing SCJVM words.
The core execution environment model has not yet had any proof
performed on it, though it has been syntax and typechecked using CZT.
