...
The core execution environment is the part of an SCJVM that is
corncerned with the execution of Java bytecode, managing the data
required to correctly handle the bytecode instructions, and
coordinating the SCJ startup and mission model.
The core execution environment model will form the starting point of
the compilation strategy.
Figure~\ref{cee-fig} shows the structure of the core execution
environment in the context of the SCJVM structure shown in
Figure~\ref{scjvm-fig}.
As can be seen from that diagram, we have divided the core execution
environment into four areas: the code area, the memory, the
interpreter and the launcher.
...
The code area is responsible for storing the bytecode instructions and
class data.
Since SCJ requires that all classes be loaded when the SCJVM starts
and that there be no dynamic class loading, the information managed by
the code area does not change throughout the execution of the SCJVM as
it is provided on startup.

The memory handles object data since the SCJVM services are agnostic
to the structure of objects and handling of objects is needed for
many bytecode instructions.
The memory must also account for entering and leaving backing stores
to ensure the correct objects are available.

The interpreter handles the execution of bytecode instructions.
This includes management of the virtual machines stacks in addition to
processing the semantics of the bytecode instructions.

The launcher coordinates the execution of bytecode instructions by the
interpreter and manages the SCJ mission model.
The launcher also provides some special handling for methods invoked
from within the interpreter that interact with the mission and memory
model.

Due to the complexity of the JVM and Java bytecode, we model a subset
of Java bytecode.
We believe it is sufficient to express a wide variety of SCJ programs
and illustrate how further features may be added, but small enough to
permit effective reasoning.
We have also made several assumptions in specifying the core execution
environment.

Our first assumption is that execeptions are not handled; errors in
the SCJVM are instead handled by simply aborting execution.
...
Another assumption we have made is that all values consist of only a
single virtual machine word.
...
instructions that handle values as object references since many of the
instructions behave the same for different types.
...
Because we are considering bytecode arising from the compilation of an
SCJ program, some requirements of SCJ permit further simplifications
to our bytecode subset.
...
Taking into account these assumptions, we have create a formal model
of the core execution environment in the \Circus{} specification
language, using one \Circus{} process for each of the four areas.
Though syntax and type checking has been performed using CZT, we have
not yet proved correctness properties of the model.
It is hoped that proofs can be done using Isabelle/UTP as part of
mechanisation of the compilation strategy.
