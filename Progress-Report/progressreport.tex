...
The first task is to define the target language for the compilation strategy.
As has been mentioned in Section~\ref{objectives-section}, the focus
is on C as the target language since that is the target language used
by several existing SCJVMs.
However, since C is being used as a target language and not as a
high-level source language, not all features of C will necessarily be required.
